...
public class ViewEventActivity extends AppCompatActivity {
    ...
    private void startSubscription() {
        ...
        Log.e(TAG, "===== Subscription started for " + event.id());
    }
    ...
    private AppSyncSubscriptionCall.Callback<NewCommentOnEventSubscription.Data> subscriptionCallback =
            new AppSyncSubscriptionCall.Callback<NewCommentOnEventSubscription.Data>() {
                ...
                public void onResponse(final @Nonnull Response<NewCommentOnEventSubscription.Data> response) {
                    Log.e(TAG, "===== Subscription response " + response);
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(ViewEventActivity.this, response.data()
                                    .subscribeToEventComments().eventId().substring(0, 5) + response.data()
                                    .subscribeToEventComments().content(), Toast.LENGTH_LONG).show();
